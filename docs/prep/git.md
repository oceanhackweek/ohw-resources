...
Each participant should clone the repository so they have their copy on their JupyterHub account space (and locally in the participant's computer, if desired). Navigate through the terminal to the folder where you want to keep {{hackweek.name}} work (`cd path_to_{{hackweek.name_lc}}`).
...
!!! warning
    Make sure your `origin` contains your github username, and upstream contains the `{{hackweek.name_lc}}` name.
